...
class HydrusRun:
    ...
    def _inputs(self):
        ...
        paste = (
            ...
            .rename(columns={c: c[5:] for c in cols if c.lower().startswith(
            'paste')}).assign(paste=True))
        ssat = (
            df[[c for c in cols if not c.lower().startswith('paste')]]
            .rename(columns={c: c[4:] for c in cols if c.lower().startswith(
            'ssat')}).assign(paste=False))
    ...
    def _run_batch(self):
        ...
        merged = (
            outputs.merge(
                inputs, on=['soil_type', 'gypsum', 'VWC'], how='left')
            .set_index('soil_type'))
        return merged

    def graph_output(self, sat_paste='gypsum', ssat='gypsum', y='EC',
                        legend = True):
        df = self._run_batch()
        groups = (
